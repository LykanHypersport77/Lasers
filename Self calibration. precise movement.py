...
ADJUSTMENT_FACTOR = 0.1  # Factor for manual adjustment
...
while True:
    ...
    if contours:
        ...
        if laser_coords:
            ...
            # Auto-calibration using simple proportional adjustment
            x_diff = laser_coords[0] - avg_coords[0]
            y_diff = laser_coords[1] - avg_coords[1]

            if abs(x_diff) > CALIBRATION_TOLERANCE:
                X_OFFSET -= x_diff * ADJUSTMENT_FACTOR  # Adjust scaling factor as needed
            if abs(y_diff) > CALIBRATION_TOLERANCE:
                Y_OFFSET -= y_diff * ADJUSTMENT_FACTOR  # Adjust scaling factor as needed
