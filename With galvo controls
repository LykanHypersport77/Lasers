# CALIBATION NOT WORKING FOR SOME REASON
import cv2
import numpy as np
import serial
...
X_OFFSET = 45  # Horizontal offset for laser targeting
Y_OFFSET = 85  # Vertical offset for laser targeting
X_SCALE = 0.4257  # Scale factor for X based on initial calibration
Y_SCALE = 0.4556  # Scale factor for Y based on initial calibration
...
def write_to_serial(port, baudrate, avg_coords):
    """
    Send commands to the galvo system to aim the laser.
    """
    try:
        ser = serial.Serial(port, baudrate, timeout=1)
        gcode_command = f"G1 X{X_OFFSET + (250 - (avg_coords[0]) * X_SCALE):.2f} Y{Y_OFFSET + (250 - (avg_coords[1] * Y_SCALE)):.2f} F15000\n"
...
# Define the color to track in BGR (Blue in this case)
...
while True:
    ...
    if contours:
        ...
        if laser_coords:
            ...
            # Auto-calibration
            x_diff = laser_coords[0] - avg_coords[0]
